...
if (isset($_GET['id'])) {
    ...
    // SQL untuk mengambil data sendi berdasarkan ID
    $sql = "SELECT * FROM sendi WHERE id='$id'";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (!empty($image)) {
        $sql = "UPDATE sendi SET name='$name', tanggal='$tanggal', description='$description', image='$image' WHERE id='$id'";
    } else {
        $sql = "UPDATE sendi SET name='$name', tanggal='$tanggal', description='$description' WHERE id='$id'";
    }

    if ($conn->query($sql) === TRUE) {
        // Mengarahkan kembali ke halaman sendi dengan pesan sukses
        header("Location: sendi.php?edit_success=true");
        exit();
    } else {
...
<body>
    <div class="container">
        ...
        <div class="main-content">
            <header>
                <h2>Edit Data Sendi</h2>
            </header>
            <div class="form-container">
                <form method="POST" action="edit-sendi.php" enctype="multipart/form-data">
                    <input type="hidden" name="id" value="<?php echo $id; ?>">
                    <div class="form-group">
                        <label for="name">Nama Sendi:</label>
